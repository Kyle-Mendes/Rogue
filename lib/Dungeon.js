...
export default class Dungeon {
	...
	addConnection(room1, room2, door, offsetX, offsetY) {
		...
		if (yOverlap.length > 2) {
			this.buildHorizontalConnection(yOverlap, room1, room2, door, offsetY);

		// If room1 and room2 share 3 X coords, make a vertical connection
		} else if (xOverlap.length > 2) {
			console.log(xOverlap)
			this.buildVerticalConnection(xOverlap, room1, room2, door, offsetX);

		// Otherwise, use a complex connection
		// @TODO allow for orientation
		} else {
			this.buildComplexConnection(room1, room2, door, offsetX, offsetY);
		}
	}
	...
	buildComplexConnection(room1, room2, door, offsetX, offsetY) {
		...
		if (door) {
			...
			sourthDoor.orientation = 'vert';
			northDoor.orientation = 'horz';
