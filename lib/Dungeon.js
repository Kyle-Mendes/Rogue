...
export default class Dungeon {
	constructor(player = new Player(0,0),
				rooms = []) {
	...
	calculateDungeonWidth() {
		let eastMostRoom = this.rooms[0];

		this.rooms.forEach(function(room) {
			if (room.xPos + room.width > eastMostRoom.xPos + eastMostRoom.width) {
				eastMostRoom = room;
			}
		});

		this.width = eastMostRoom.width + eastMostRoom.xPos;
	}
	...
	calculateDungeonHeight() {
		let southMostRoom = this.rooms[0];

		this.rooms.forEach(function(room) {
			if (room.yPos + room.height > southMostRoom.yPos + southMostRoom.height) {
				southMostRoom = room;
			}
		});

		this.height = southMostRoom.height + southMostRoom.yPos;
	}

	addConnection(room1, room2, options = {}) {
		...
		if (yOverlap.length > 2) {
			this.buildHorizontalConnection(yOverlap, room1, room2, options.door, options.offsetY);

		// If room1 and room2 share 3 X coords, make a vertical connection
		} else if (xOverlap.length > 2) {
			console.log(xOverlap)
			this.buildVerticalConnection(xOverlap, room1, room2, options.door, options.offsetX);

		// Otherwise, use a complex connection
		// @TODO allow for orientation
		} else {
			this.buildComplexConnection(room1, room2, options.door, options.offsetX, options.offsetY);
		}
	}

	buildHorizontalConnection(overlap, room1, room2, door, offsetY) {
		let tallerRoom,
			shorterRoom,
			rightMostRoom,
			leftMostRoom,
		...
		if (door) {
			door.setOrientation('horz')
			leftMostRoom.foundation[startX + 1][startY] = door;
			rightMostRoom.foundation[startX + 1][endY] = door;
		} else {
			// Make a whole in the room to for the door to the corridor
			leftMostRoom.foundation[startX][startY + 1] = new Door(true, 'empty');
			rightMostRoom.foundation[endX][startY + 1] = new Door(true, 'empty');
		}
	...
	buildVerticalConnection(overlap, room1, room2, door, offsetX) {
		let widerRoom,
			thinnerRoom,
			northMostRoom,
			southMostRoom,
		...
		if (door) {
			door.setOrientation('vert')
			northMostRoom.foundation[startX + 1][startY] = door;
			southMostRoom.foundation[startX + 1][endY] = door;
		} else {
			// Make a whole in the room to for the door to the corridor
			northMostRoom.foundation[startX + 1][startY] = new Door(true, 'empty');
			southMostRoom.foundation[startX + 1][endY] = new Door(true, 'empty');
		}
	...
	buildComplexConnection(room1, room2, door, offsetX, offsetY) {
		let northMostRoom,
			southMostRoom,
			startX,
			xDirection;
		...
		if (door) {
			let southDoor = new Door(door.open, door.type, door.characters),
				northDoor = new Door(door.open, door.type, door.characters);

			southDoor.orientation = 'horz';
			northDoor.orientation = 'vert';

			southMostRoom.foundation[startX][intersection.y] = southDoor;
			northMostRoom.foundation[intersection.x][startY] = northDoor;
		} else {
			// Make a whole in the room to for the door to the corridor
			southMostRoom.foundation[startX][intersection.y] = new Door(true, 'empty');
			northMostRoom.foundation[intersection.x][startY] = new Door(true, 'empty');
		}
